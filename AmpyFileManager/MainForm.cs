...
namespace AmpyFileManager
{
    public partial class frmMain : Form
    {
        ...
        private void frmMain_Load(object sender, EventArgs e)
        {
            ...
            // Setup tooltips
            toolTip1.SetToolTip(btnREPL, "打开终端窗口");
            toolTip1.SetToolTip(btnDelete, "永久删除选定的文件或目录的设备");
            toolTip1.SetToolTip(btnExport, "导出所选文件到你的电脑设备");
            toolTip1.SetToolTip(btnLoad, "从电脑导入文件到设备");
            toolTip1.SetToolTip(btnMkdir, "在设备中创建一个子目录");
            toolTip1.SetToolTip(btnMove, "移动选中文件");
            toolTip1.SetToolTip(btnNew, "创建新的文件");
            toolTip1.SetToolTip(btnOpen, "打开选择文件进行编辑或目录以供查看");
            toolTip1.SetToolTip(btnRefresh, "刷新设备中的文件");
            toolTip1.SetToolTip(btnRun, "运行当前选中的文件");
            toolTip1.SetToolTip(btnSave, "保存当前文件");
            toolTip1.SetToolTip(btnSaveAs, "保存当前文件到当前目录指定名称");
            toolTip1.SetToolTip(btnReplaceAll, "简单的搜索替换为当前文件");
            toolTip1.SetToolTip(cboHelp, "帮助文档");
            toolTip1.SetToolTip(btnBackupScript, "生成一个DOS批处理文件,将备份当前设备的内容。");
            toolTip1.SetToolTip(btnRestoreScript, "生成一个DOS批处理文件,将恢复之前备份。");
            ...
            if (selectedItem != "")
            {
                if (selectedItem.StartsWith(LBracket))
                {
                    ...
                    if (DirToDelete != "..")
                    {
                        ...
                        if (MessageBox.Show("你确定你要删除目录吗'" + FullDirToDelete + "' 和它的所有内容吗?", "确认删除", MessageBoxButtons.YesNoCancel) == DialogResult.Yes)
                        {
                            Cursor.Current = Cursors.WaitCursor;
                ...
                else
                {
                    ...
                    if (MessageBox.Show("你确定你要删除吗 '" + FileToDelete + "'?", "确定删除", MessageBoxButtons.YesNoCancel) == DialogResult.Yes)
                    {
                        Cursor.Current = Cursors.WaitCursor;
        ...
        private void btnMove_Click(object sender, EventArgs e)
        {
            ...
            if (selectedItem != "")
            {
                if (!selectedItem.StartsWith(LBracket))
                {
                    ...
                    MessageBox.Show("只能移动文件.", "不支持");
            }

            if (FileToMove != "")
            {
                string filename = Microsoft.VisualBasic.Interaction.InputBox("新路径和文件名:", "移动文件", "");
                if (filename != "")
                {
            ...
            if (selectedItem != "")
            {
                if (!selectedItem.StartsWith(LBracket))
                {
                    if (selectedItem.EndsWith(".py"))
                        ...
                        MessageBox.Show("只能运行 .py 文件。", "不支持");
                }
                else
                    MessageBox.Show("只能运行 .py 文件。", "不支持");
            }
            ...
            if (selectedItem != "")
            {
                ...
                else // Otherwise open the file
                {
                    ...
                    if (EditableFile(LocalFile))
                    {
                        ...
                        MessageBox.Show("不列为一个可编辑的文件类型。添加更多扩展的. config文件。");
                }
            }
        }
        ...
        private bool OKToContinue()
        {
            ...
            if (_FileDirty)
            {
                DialogResult r = MessageBox.Show("文件已经被编辑。你想要保存它吗?", "确定", MessageBoxButtons.YesNoCancel);
        ...
        private bool DoSave(string prefill = "")
        {
            ...
            if (_CurrentFile == NEW_FILENAME)
            {
                ...
                string filename = Microsoft.VisualBasic.Interaction.InputBox("新文件名:", "保存文件", justfile);
                if (filename != "")
                {
                    if (filename.IndexOf(".") > 0)
                    {
